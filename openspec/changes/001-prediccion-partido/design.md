...
- ≤3 módulos: todo vive en `backend/app/`, `backend/ml/`, `backend/workers/` del monorepo existente — sin proyectos nuevos.
...
- **Integridad temporal:** entrenamiento con split cronológico (test existente en `backend/ml/evaluation/`); calibración del badge de confianza usa solo backtesting cronológico, nunca resultados futuros.
- **Independencia del mercado:** el feature set no incluye columnas de odds (test existente en `backend/ml/features/`); esta capability no consume cuotas de mercado en absoluto.
- **Aislamiento de procesos:** generación de predicciones vía `backend/workers/tasks/generate_predictions.py` (Celery); `backend/app/api/matches` es de solo lectura.
