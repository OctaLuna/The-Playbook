...
class EvidenceOut(BaseModel):
    ...
    published_at: datetime = Field(
        ..., description="Fecha de publicación UTC, siempre previa al kickoff"
    )
...
class ExplanationOut(BaseModel):
    ...
    is_fallback_no_evidence: bool = Field(
        False,
        description="True si no hubo evidencia suficiente y se usó el mensaje de ausencia de datos",
    )
    evidence: list[EvidenceOut] = Field(
        default_factory=list, description="Lista ordenada de fuentes citadas"
    )
...
class FollowUpRequest(BaseModel):
    ...
    question: str = Field(
        ..., min_length=3, max_length=500, description="Pregunta de seguimiento del usuario"
    )
