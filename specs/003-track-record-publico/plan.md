...
**Stack propuesto por el usuario:** El definido en `docs/project_spec.md` (secciones 4, 5 y 9) — FastAPI + SQLAlchemy 2.0 async sobre PostgreSQL 16, Celery + Redis para el recálculo periódico, reutilizando `ml/evaluation/` (log-loss, Brier score, backtesting cronológico) ya existente para 001.
...
- [x] ¿Se usa el framework directamente? Sí — se reutilizan las funciones de `ml/evaluation/` (log-loss, Brier) ya existentes para 001, en vez de reimplementar una segunda vez las métricas para el track record.
...
- [x] **Artículo VI:** el recálculo del agregado corre vía Celery (`workers/tasks/update_track_record.py`, job diario ya resuelto en Clarify); el endpoint `GET /api/track-record` solo lee la tabla `TrackRecordAgregado` ya calculada.
...
| Reutilizar `ml/evaluation/` para Brier/log-loss en vez de reimplementar | Implementar cálculo de métricas propio para el track record | Garantiza que el panel público sea *exactamente* la misma métrica que usa el equipo internamente (CE-001); evita duplicar lógica ya cubierta por el test de split cronológico | RF-002, CE-001 |
...
2. Tests, en este orden:
   ...
   - Pruebas unitarias de la reutilización de `ml/evaluation/` para el cálculo por partido
3. Código fuente, en este orden: modelos SQLAlchemy (`EvaluaciónPredicción`, `TrackRecordAgregado`, `CuotaMercado`) → `app/services/track_record_service.py` (usa `ml/evaluation/`) → `workers/tasks/update_track_record.py` → routers de `app/api/track_record`.
