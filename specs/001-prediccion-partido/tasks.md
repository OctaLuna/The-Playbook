...
- [ ] T001 [P] Prueba de contrato GET `/api/leagues` en `backend/tests/contract/test_leagues.py`
- [ ] T002 [P] Prueba de contrato GET `/api/matches/upcoming` (con filtro `league` y paginación) en `backend/tests/contract/test_matches_upcoming.py`
- [ ] T003 [P] Prueba de contrato GET `/api/matches/{match_id}` (incluye caso 404) en `backend/tests/contract/test_match_detail.py`
- [ ] T004 [P] Prueba de contrato GET `/api/matches/{match_id}/prediction` (incluye caso 404 sin predicción generada) en `backend/tests/contract/test_match_prediction.py`
- [ ] T005 Confirmar que T001-T004 fallan (fase Red) antes de continuar

## Grupo 2 — Modelo de datos
- [ ] T006 [P] Crear modelo SQLAlchemy `Partido` en `backend/app/models/partido.py` (enum `liga` con las 5 ligas, enum `estado`: programado/jugado/pospuesto/cancelado)
- [ ] T007 [P] Crear modelo SQLAlchemy `Equipo` en `backend/app/models/equipo.py` (incluye `tiene_historial_suficiente`)
- [ ] T008 [P] Crear modelo SQLAlchemy `Predicción` en `backend/app/models/prediccion.py` (incluye `top_shap_features` como campo interno JSON, `version_modelo`)
- [ ] T009 [P] Crear modelo SQLAlchemy `CalibraciónHistórica` en `backend/app/models/calibracion_historica.py`
- [ ] T010 Migración Alembic inicial para las 4 tablas (depende de T006-T009)
...
- [ ] T018 Prueba de integración: Historia 1, escenario 2 — equipo con `tiene_historial_suficiente = false` devuelve `low_data_warning: true`
