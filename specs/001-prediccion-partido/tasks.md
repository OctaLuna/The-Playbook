...
- [x] T012b Implementar `backend/ml/data/`: carga de Football-Data.co.uk (`loader.py`), tabla de alias de equipo (`alias.py`), y persistencia de Partido/Equipo históricos (`persist.py`) según el mapeo de columnas de `ml-design.md` §1 (falla explícito ante un nombre de equipo sin alias, nunca crea un `Equipo` duplicado) — [RF-008]
